...
from MolecularDiffusion.modules.layers.equiformer_v2.wigner import wigner_D
...
class CoefficientMappingModule(torch.nn.Module):
    ...
    def complex_idx(self, m, lmax, m_complex, l_harmonic):
        """Add `m_complex` and `l_harmonic` to the input arguments since we cannot use
        `self.m_complex`."""
        if lmax == -1:
...
class SO3_Embedding:
    ...
    def _grid_act(self, SO3_grid, act, mappingReduced):
        ...
        for i in range(self.num_resolutions):
            ...
            to_grid_mat = SO3_grid[self.lmax_list[i]][self.mmax_list[i]].get_to_grid_mat(
                self.device
            )
            from_grid_mat = SO3_grid[self.lmax_list[i]][self.mmax_list[i]].get_from_grid_mat(
                self.device
            )
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}(length={self.length}, lmax_list={self.lmax_list}, num_channels={self.num_channels})"
