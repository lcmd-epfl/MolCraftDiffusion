...
from MolecularDiffusion.modules.layers.equiformer_v2.so3 import SO3_Embedding
...
class SO2_Convolution(torch.nn.Module):
    ...
    def forward(self, x, x_edge):
        ...
        out_embedding = SO3_Embedding(
            0, x.lmax_list.copy(), self.m_output_channels, device=x.device, dtype=x.dtype
        )
...
class SO2_Linear(torch.nn.Module):
    ...
    def __init__(
        ...
        # SO(2) linear for m = 0
        self.fc_m0 = Linear(
            num_channels_m0, self.m_output_channels * (num_channels_m0 // self.sphere_channels)
        )
        num_channels_rad = num_channels_rad + self.fc_m0.in_features
