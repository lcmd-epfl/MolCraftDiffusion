        ...
        if use_pbc:
            if otf_graph:
                if use_pbc_single:
                    ...
                    atom_index_offset = data.num_atoms.cumsum(dim=0).roll(1)
            ...
            if otf_graph:
                if fully_connected:
                    ...
                    adj = torch.ones(data.num_atoms.sum(), data.num_atoms.sum(), device=data.pos.device) - torch.eye(data.num_atoms.sum(), device=data.pos.device)
                    # Use batch to mask out edges between different molecules
                    batch_mask = data.batch.unsqueeze(0) == data.batch.unsqueeze(1)
                    adj = adj * batch_mask
...
def radius_graph_pbc(
    ...
    num_atoms_per_image = data.num_atoms
