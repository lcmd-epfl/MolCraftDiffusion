...
class Trans_Layer(MessagePassing):
    ...
    def forward(self, x: OptTensor,
        ...
        # PORT FIX: upstream's propagate_type comment listed (x, edge_attr),
        # which does not match the kwargs actually passed. Older PyG ignored
        # the comment; PyG >= 2.5 code-generates `propagate` from it, so the
        # stale annotation made every call a TypeError. Corrected to the real
        # message() signature; no behaviour change.
...
class TransMixLayer(MessagePassing):
    ...
    def forward(self, x: OptTensor,
        ...
        # stale annotation made every call a TypeError. Corrected to the real
        # message() signature; no behaviour change.
        # propagate_type: (query: Tensor, key: Tensor, value: Tensor, edge_attr: OptTensor, extra_heads: OptTensor)
