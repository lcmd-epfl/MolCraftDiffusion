...
from MolecularDiffusion.modules.layers.gfmdiff.blocks import (
    InterBlock,
    DistEncoder,
    RBF_Emb,
    atom_pos_to_pair_dist,
...
class EquiGNN(nn.Module):
    def __init__(self, model_config, data_config):
        ...
        # ponytail: degree/valence channel dropped per approved plan
        # (Geometric-Facilitated Loss out of scope) — hardcoded, not read
        # from model_config.
    ...
    def _forward(self, z_t, t, node_mask, pair_mask, context):
        ...
        pair_emb = self.node_pair_fuse(
            torch.cat(
                [xh_emb.unsqueeze(1) * pair_mask, xh_emb.unsqueeze(2) * pair_mask, pair_emb],
                dim=3,
            )
        )
        ...
        if not self.add_time:
            ...
            diag = torch.ones(
                (node_mask.shape[1], node_mask.shape[1], node_mask.shape[1]),
            ...
            if self.block_calc:
                ...
                if self.add_time:
                    tri_emb = self.act(
                        self.trip_emb_fuse(
                            torch.cat(
                                [self.angle_emb(angle, xh_time.unsqueeze(-1) * triplet_mask), tri_emb],
                                dim=4,
                            )
                        )
                    )
                    pair_emb = self.act(
                        self.edge_emb_fuse(
                            torch.cat(
                                [self.edge_emb(radial, xh_time.unsqueeze(-1) * pair_mask), pair_emb],
                                dim=3,
                            )
                        )
                    )
                    ...
                    tri_emb = self.act(
                        self.trip_emb_fuse(torch.cat([self.angle_emb(angle), tri_emb], dim=4))
                    )
                    pair_emb = self.act(
                        self.edge_emb_fuse(torch.cat([self.edge_emb(radial), pair_emb], dim=3))
                    )
            pair_emb = self.node_pair_fuse(
                torch.cat(
                    [xh_emb.unsqueeze(1) * pair_mask, xh_emb.unsqueeze(2) * pair_mask, pair_emb],
