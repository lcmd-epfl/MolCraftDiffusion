...
# upstream path: syncogen/models/semla_pharm.py
...
class SemlaPharmGenerator(nn.Module):
    ...
    def forward(
        ...
        # UPSTREAM DEFECT (semla_pharm.py:1014): this cat ran unconditionally,
        # but the branch above sets cond_coords = None when self-conditioning is
        # off, so torch.cat raised. Latent upstream only because every shipped
        # experiment sets Diffusion.self_conditioning = True. Guarded here; the
        # True path is byte-identical, so the released conditional checkpoint
        ...
        if edge_feats is not None:
            # UPSTREAM DEFECT (semla_pharm.py:1111): `.to(torch.float16)` was
            # hardcoded while edge_feats was allocated fp32 and both shipped
            # experiments run Trainer.precision = '32'. edge_feats is a 0/1
            # one-hot bond tensor, exactly representable in fp16, so following
            # the surrounding activations changes no value -- it only removes a
            # dtype mismatch against fp32 cond_bonds in the cat below.
            edge_feats = edge_feats.to(X.dtype)
            edge_feats = (
