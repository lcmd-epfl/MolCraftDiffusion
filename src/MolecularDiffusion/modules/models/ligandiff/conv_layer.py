...
from MolecularDiffusion.modules.models.ligandiff.dropout import GVPDropout
from MolecularDiffusion.modules.models.ligandiff.gvp import GVP, s_V
from MolecularDiffusion.modules.models.ligandiff.layer_norm import GVPLayerNorm
...
class GVPMessagePassing(MessagePassing, ABC):
    ...
    def forward(
        self, x: s_V, edge_index: torch.Tensor, edge_attr: torch.Tensor
    ) -> s_V:
        s, V = x
        v_dim = V.shape[-1]
        V = torch.flatten(V, start_dim=-2, end_dim=-1)
        return self.propagate(
            edge_index, s=s, V=V, edge_attr=edge_attr, v_dim=v_dim
        )
    ...
    def update(self, aggr_out: torch.Tensor) -> s_V:
        s_aggr, V_aggr = self._split(
            aggr_out, self.out_scalar, self.out_vector
        )
...
class GVPConvLayer(GVPMessagePassing, ABC):
    ...
    def __init__(
        ...
        self.dropout = nn.ModuleList(
            [GVPDropout(drop_rate) for _ in range(2)]
        )
