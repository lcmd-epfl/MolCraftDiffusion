...
Verbatim port of LigandDiff's ``src/gvp.py`` (itself from DiffHopp,
...
class GVP(nn.Module):
    """Geometric vector perceptron (Jing et al.)."""
    ...
    def forward(
        ...
        s, V = (
            x
            if self.v > 0
            else (x, torch.empty((x.shape[0], 0, 3), device=x.device))
        )
        ...
        if self.vector_gate:
            V_dash = (
                self.sigma_g(self.W_g(self.sigma_plus(s_m)))[..., None] * V_mu
            )
        else:
            v_mu = torch.clip(
                torch.norm(V_mu, dim=-1, keepdim=True), min=self.eps
            )
