...
from MolecularDiffusion.modules.models.shepherd_arch.egnn import EGNN, MultiLayerPerceptron, GaussianSmearing
...
from MolecularDiffusion.modules.models.shepherd_arch.utils import add_virtual_edges_to_edge_index
from MolecularDiffusion.modules.models.shepherd_arch.utils import positional_encoding

import e3nn
from MolecularDiffusion.modules.models.shepherd_arch.equiformer_operations import FeedForwardNetwork_equiformer, convert_e3nn_to_equiformerv2, convert_equiformerv2_to_e3nn
...
class Model(torch.nn.Module):
    ...
    def forward_x1_decoder_encoder(self, input_dict, output_dict):
        ...
        _x1_max_nn = self.params['x1']['decoder']['encoder']['max_neighbors'] if 'max_neighbors' in self.params['x1']['decoder']['encoder'] else 1000000
        edge_index = radius_graph(
            ...
            max_num_neighbors = min(_x1_max_nn, len(input_dict['x1']['decoder']['pos']) - 1),
        )
    ...
    def forward_x2_decoder_encoder(self, input_dict, output_dict):
        ...
        _x2_max_nn = self.params['x2']['decoder']['encoder']['max_neighbors'] if 'max_neighbors' in self.params['x2']['decoder']['encoder'] else 1000000
        edge_index = radius_graph(
            input_dict['x2']['decoder']['pos'],
            r = self.params['x2']['decoder']['encoder']['cutoff'],
            batch = input_dict['x2']['decoder']['batch'],
            max_num_neighbors = min(_x2_max_nn, len(input_dict['x2']['decoder']['pos']) - 1),
        )
    ...
    def forward_x3_decoder_encoder(self, input_dict, output_dict):
        ...
        _x3_max_nn = self.params['x3']['decoder']['encoder']['max_neighbors'] if 'max_neighbors' in self.params['x3']['decoder']['encoder'] else 1000000
        edge_index = radius_graph(
            input_dict['x3']['decoder']['pos'],
            r = self.params['x3']['decoder']['encoder']['cutoff'],
            batch = input_dict['x3']['decoder']['batch'],
            max_num_neighbors = min(_x3_max_nn, len(input_dict['x3']['decoder']['pos']) - 1),
        )
    ...
    def forward_x4_decoder_encoder(self, input_dict, output_dict):
        # If all nodes are virtual (no real pharmacophores), skip encoder
        # and produce zero global embedding
        virtual_node_mask = input_dict['x4']['decoder']['virtual_node_mask']
        if virtual_node_mask.all():
            num_graphs = max(input_dict['x4']['decoder']['batch']) + 1
            node_channels = self.params['x4']['decoder']['encoder']['sphere_channels']
            lmax_list = self.params['x4']['decoder']['encoder']['lmax_list']
            num_coefficients = sum([(2 * l + 1) for l in range(max(lmax_list) + 1)])
            zero_global = SO3_Embedding(num_graphs, lmax_list, node_channels, self.device, self.dtype)
            zero_node = SO3_Embedding(
                input_dict['x4']['decoder']['pos'].shape[0], lmax_list, node_channels, self.device, self.dtype
            )
            output_dict['x4']['decoder']['encoder']['edge_index'] = torch.zeros(2, 0, dtype=torch.long, device=self.device)
            output_dict['x4']['decoder']['encoder']['node_embedding'] = zero_node
            output_dict['x4']['decoder']['encoder']['global_embedding'] = zero_global
            return output_dict
        ...
        _x4_max_nn = self.params['x4']['decoder']['encoder']['max_neighbors'] if 'max_neighbors' in self.params['x4']['decoder']['encoder'] else 1000000
        edge_index = radius_graph(
            input_dict['x4']['decoder']['pos'],
            r = self.params['x4']['decoder']['encoder']['cutoff'],
            batch = input_dict['x4']['decoder']['batch'],
            max_num_neighbors = min(_x4_max_nn, len(input_dict['x4']['decoder']['pos']) - 1),
        )
    ...
    def forward_decoder_joint_heterogeneous_graph_encoder(self, input_dict, output_dict):
        ...
        hetero_edge_index = radius_graph(
            ...
            max_num_neighbors = len(hetero_pos) - 1,
        )
