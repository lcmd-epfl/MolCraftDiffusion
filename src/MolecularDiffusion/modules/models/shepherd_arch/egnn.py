...
class EGNN(torch.nn.Module):
    def __init__(
        ...
        self.node_output_embedding = torch.nn.Linear(
            self.node_mlp_embedding_dim, self.node_output_embedding_dim
        )
    ...
    def forward(
        ...
        if self.normalize_distance_vectors:
            edge_distance_vec = edge_distance_vec / (
                torch.linalg.norm(edge_distance_vec, dim=-1, keepdim=True) + 1e-6
            )

        edge_weights = self.coordinate_mlp(messages)
