...
def unsorted_segment_sum(data, segment_ids, num_segments, normalization_factor, aggregation_method: str):
    """Replicates TensorFlow's ``unsorted_segment_sum``. ``aggregation_method``
    is 'sum' or 'mean'."""
...
class Dynamics(nn.Module):
    def __init__(
        ...
        super().__init__()
        # ponytail: no `self.device` attribute here -- `get_edges` below
        # reads the live device off a registered parameter instead, so a
        # constructor-time-only device string would be redundant (and, if
        # ever read on a moved module, stale/misleading). `device` is still
        # used below to place the freshly-constructed `EGNN`/`GNN` submodule.
        self.n_dims = n_dims
    ...
    def get_edges(self, n_nodes, batch_size):
        # ponytail: read the live device off a registered parameter instead
        # of the plain `self.device` attribute set once at construction --
        # `Engine.__init__` moves this module via `nn.Module.cuda()`, which
        # updates parameters/buffers through `_apply()` without dispatching
        # through any subclass-overridden `to()`, so `self.device` goes
        # stale on GPU runs (see INTEGRATION_PLAN.md Tester Log, attempt 1).
        device = next(self.parameters()).device
        cache_key = (n_nodes, batch_size)
        if cache_key in self.edge_cache:
            cached_device, edges = self.edge_cache[cache_key]
...
class DynamicsWithPockets(Dynamics):
    ...
    def forward(self, t, xh, node_mask, linker_mask, edge_mask, context):
        ...
        fragment_only_mask = context[..., -2].view(bs * n_nodes, 1)
        pocket_only_mask = context[..., -1].view(bs * n_nodes, 1)
        assert torch.all(
            fragment_only_mask.bool() | pocket_only_mask.bool() | linker_mask.bool() == node_mask.bool()
        )
        ...
        if self.model == "egnn_dynamics":
            h_final, x_final = self.dynamics(
                h, x, edges, node_mask=node_mask, linker_mask=linker_mask, edge_mask=None
            )
            vel = (x_final - x) * node_mask
        elif self.model == "gnn_dynamics":
            xh = torch.cat([x, h], dim=1)
