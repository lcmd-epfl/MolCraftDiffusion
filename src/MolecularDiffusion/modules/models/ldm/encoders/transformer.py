...
class TransformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        # Atom type embedding (direct atomic number -> embedding)
        self.atom_type_embedder = nn.Embedding(max_num_elements, d_model)
        
        # Cartesian coordinate embedder
        ...
        # Fractional coordinate embedder (silenced for molecules - receives zeros)
    ...
    def forward(self, batch) -> Dict[str, torch.Tensor]:
        """
        Args:
            batch: PyG Data object with the following attributes:
                atom_types (torch.Tensor): Atomic numbers of atoms (n,)
                pos (torch.Tensor): Cartesian coordinates (n, 3)
                frac_coords (torch.Tensor): Fractional coordinates (n, 3) - zeros for molecules
                num_atoms (torch.Tensor): Number of atoms per sample (batch_size,)
                batch (torch.Tensor): Batch index for each atom (n,)
                token_idx (torch.Tensor): Token index within each molecule (n,)

        Returns:
            Dict with keys: x (n, d), num_atoms, batch, token_idx
        """
        x = self.atom_type_embedder(batch.atom_types)  # (n, d)
        x = x + self.pos_embedder(batch.pos)
        x = x + self.frac_coords_embedder(batch.frac_coords)

        # Positional embedding based on token index
        x = x + get_index_embedding(batch.token_idx, self.d_model)
