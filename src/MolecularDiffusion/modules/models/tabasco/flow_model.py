...
from MolecularDiffusion.modules.models.tabasco.flow.interpolate import Interpolant
from MolecularDiffusion.modules.models.tabasco.flow.path import FlowPath
from MolecularDiffusion.modules.models.tabasco.flow.utils import HistogramTimeDistribution
from MolecularDiffusion.modules.layers.tabasco.losses import InterDistancesLoss
from MolecularDiffusion.modules.models.tabasco.data.transforms import apply_random_rotation
...
class FlowMatchingModel(nn.Module):
    ...
    def _sample_noise_like_batch(
        ...
        if batch is None:
            ...
            histogram = self.data_stats["num_atoms_histogram"]
            
            if not histogram:
                # Fallback to uniform distribution up to max_num_atoms
                # (Assuming at least 1 atom)
                histogram = {i: 1.0 for i in range(1, max_num_atoms + 1)}
